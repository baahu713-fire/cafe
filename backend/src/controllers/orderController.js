...
const createOrder = async (req, res) => {
  try {
    ...
    if (isAdmin && userIdToOrderFor) {
      ...
      createdByAdmin = requestingUser.id; // Track which admin created the order
    }

    const order = await orderService.createOrder(orderData, finalUserId, createdByAdmin);
...
const getMyOrders = async (req, res) => {
  try {
    ...
    const result = await orderService.getOrdersByUserId(userId, page, limit, startDate, endDate);
    res.json(result);
  } catch (error) {
...
const getDailySummary = async (req, res) => {
  try {
    ...
    const summary = await orderService.getDailyItemSummary(date);
    res.json(summary);
  } catch (error) {
